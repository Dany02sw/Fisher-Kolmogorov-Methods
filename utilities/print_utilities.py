from utilities.enum_utilities       import SpaceMethod, TimeMethod
from utilities.math_utilities       import compute_rate, compute_exponential_fit
from utilities.dictionary_utilities import ERROR_LABELS_PRINT
...
def print_space_rates(errors_c, errors_grad, hs, N_list, l, method=SpaceMethod.LDG):
    ...
    print(f"\n{header:=^120}")
    print(f"\n{'N_from(h_from)':>16} → {'N_to(h_to)':>16} | {label_c+' from':>12} {label_c+' to':>12} {'rate':>6} {'(expc)':>5} | {label_grad+' from':>12} {label_grad+' to':>12} {'rate':>6} {'(expc)':>5}")
    print("-"*120)
    for i in range(1, len(N_list)):
        rate_c    = compute_rate(errors_c,    hs, i)
        rate_grad = compute_rate(errors_grad, hs, i)
        col_from  = f"{N_list[i-1]}({hs[i-1]:.4f})"
        col_to    = f"{N_list[i]}({hs[i]:.4f})"
        print(f"{col_from:>16} → {col_to:>16} | {errors_c[i-1]:>12.4e} {errors_c[i]:>12.4e} {rate_c:>6.2f} ({l+1:.2f}) | {errors_grad[i-1]:>12.4e} {errors_grad[i]:>12.4e} {rate_grad:>6.2f} ({l:.2f})")
