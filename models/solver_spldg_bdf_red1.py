from models.solver_bdf import SolverBDF

from dolfin import *
import ufl

from utilities.enum_utilities      import SpaceMethod
from utilities.transform_utilities import Sigmoid
from utilities.spldg_utilities     import div_LDG, grad_LDG, inner_LDG

class SolverSpLdgBDFReduced1(SolverBDF):
    ...
    def _SolvePostprocessing(self, t_val):
        # Extract solution
        w_h = self.U.split(deepcopy=True)
    ...
    def _ConvergenceTestPostprocessing(self, t_val):
        ...
        E_LDG = sqrt(assemble(inner(grad(self.c_ex) + sigma_h, grad(self.c_ex) + sigma_h)*self.dx))
